...
/**
 * 封装sqlSessionTemplate的代码，Dao继承BaseDaoIml【偷懒了这个步骤】
 * @author danielpeter
 *
 */
...
public class BaseDaoImpl extends BaseClass{
	 ...
	 * @return Object 
	 */
	public PageGrid pageForObject(String statementName,PageQueryModel parameterObject){
		int records = 0;
		...
		if(queryModel == null){
			//不传 PageQueryModel参数，默认查询全部，不计数
		...
		queryParam.put("doCount", queryModel.getDoCount());
		queryParam.put("startPosition", StringUtil.getString(queryModel.getStartPosition()));
		queryParam.put("pageSize", StringUtil.getString(queryModel.getPageSize()));
